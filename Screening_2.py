...
#------------------------------------------------------------------------------
# ============================================================================
# COMPLETE ORIGINAL CODE OF SCREENING_2.PY (SCREENING GUI)
# ============================================================================
"""
...
class Ui_Screening(object):
    def setupUi(self, Screening, selected_module="PyRMD2Dock"):
        ...
        self.lineEdit_program_mode.setText("Structure-Based Virtual Screening (SBVS), Screening")
        ...
        self.pushButton_db_to_screen.clicked.connect(lambda: self.browse_file("Browse_File_to_Screen", self.lineEdit_db_to_screen, "Select Database File"))
        ...
        self.lineEdit_output_file.setText("PyRMD2Dock_database_predictions.csv")
        ...
        self.pushButton_decoys_file.setToolTip("Select a file containing decoys for the screening process.")
        ...
        self.lineEdit_beta.setToolTip("Enter the alpha value for statistical calculations.")
    ...
    def _toggle_training_uploads(self):
        """Always show Actives/Inactives file upload fields"""
        # ChEMBL fields removed → no need to toggle
        # Only show Actives and Inactives
    ...
    def update_ini_file(self):
        ...
        try:
            ...
            if output_dir and output_file:
                ...
                full_output_path = "PyRMD2Dock_database_predictions.csv"
            ...
            config.set('TRAINING_DATASETS', 'use_chembl', 'false')
            config.set('TRAINING_DATASETS', 'chembl_file', '')
            config.set('TRAINING_DATASETS', 'use_actives', 'true')
            config.set('TRAINING_DATASETS', 'actives_file', self.lineEdit_actives_file.text())
            config.set('TRAINING_DATASETS', 'use_inactives', 'true')
            ...
            # FILTER section
            filter_enabled = self.checkBox_use_filter.isChecked()
            config.set('FILTER', 'filter_properties', str(filter_enabled).lower())

            for label_text, lineEdit in self.filter_fields.items():
                # normalize key names for config
                key = label_text.lower().replace(" ", "_").replace(":", "")
                config.set('FILTER', key, lineEdit.text().strip())
    ...
    def load_default_values(self):
        ...
        try:
            ...
            # Training datasets (always actives/inactives, no chembl)
    ...
    def _shared_output_settings_path(self):
        return os.path.abspath("screening_output_preferences_pyrmd2dock.json")
...
class UnifiedTabApplication(QMainWindow):
    ...
    def initUI(self):
        ...
        # Set window properties
        self.setWindowTitle("PyRMD2Dock Screening and Analysis")
...
def main_unified():
    ...
    # Set application properties
    app.setApplicationName("Unified GUI Application")
